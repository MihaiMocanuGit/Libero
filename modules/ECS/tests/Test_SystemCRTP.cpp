...
class Derived_int : public SystemCRTP<Derived_int>
{
  ...
  private:
    int run_crtp(uint64_t, [[maybe_unused]] int dummy) { return ++test; }
    friend Base;
};
...
class Derived_void : public SystemCRTP<Derived_void>
{
  ...
  private:
    void run_crtp(uint64_t, [[maybe_unused]] int dummy)
    {
        test++;
        return;
    }
...
class Derived_intCRef : public SystemCRTP<Derived_intCRef>
{
  ...
  private:
    const int &run_crtp(uint64_t, [[maybe_unused]] int dummy) { return ++test; }
    friend Base;
};
...
class Derived_intCRefC : public SystemCRTP<Derived_intCRefC>
{
  ...
  private:
    const int &run_crtp(uint64_t, [[maybe_unused]] int dummy) const { return ++test; }
    friend Base;
};
...
TEST_CASE("SystemCRTP usage", "[ECS][SystemCRTP]")
{
    ...
    REQUIRE(dint.run(0, 0) == 1);
    // second run is too soon to be called
    REQUIRE(not dint.run(1, 0).has_value());
    REQUIRE(dint.test == 1);
    // this is the last time it won't be called
    REQUIRE(not dint.run(99, 0).has_value());
    REQUIRE(dint.test == 1);
    // this is the first time it will be called
    REQUIRE(dint.run(100, 0) == 2);
    ...
    REQUIRE(dintRef.run(0, 0)->get() == 1);
    // second run is too soon to be invoked
    REQUIRE(not dintRef.run(1, 0).has_value());
    REQUIRE(dintRef.test == 1);
    // this is the last time it won't be invoked
    REQUIRE(not dintRef.run(99, 0).has_value());
    REQUIRE(dintRef.test == 1);
    // this is the first time it will be invoked
    REQUIRE(dintRef.run(100, 0)->get() == 2);

    Derived_intCRef dintCRef {100};
    // first run is always invoked
    REQUIRE(dintCRef.run(0, 0)->get() == 1);
    // second run is too soon to be invoked
    REQUIRE(not dintCRef.run(1, 0).has_value());
    REQUIRE(dintCRef.test == 1);
    // this is the last time it won't be invoked
    REQUIRE(not dintCRef.run(99, 0).has_value());
    REQUIRE(dintCRef.test == 1);
    // this is the first time it will be invoked
    REQUIRE(dintCRef.run(100, 0)->get() == 2);

    Derived_intCRefC dintCRefC {100};
    // first run is always invoked
    REQUIRE(dintCRefC.run(0, 0)->get() == 1);
    // second run is too soon to be invoked
    REQUIRE(not dintCRefC.run(1, 0).has_value());
    REQUIRE(dintCRefC.test == 1);
    // this is the last time it won't be invoked
    REQUIRE(not dintCRefC.run(99, 0).has_value());
    REQUIRE(dintCRefC.test == 1);
    // this is the first time it will be invoked
    REQUIRE(dintCRefC.run(100, 0)->get() == 2);
}
